GalleryGuard — “Invisible buffer for priceless art.”
  ...
  - Reliability (30%): Simple distance threshold + stable climate ranges; easy to tune;
    very low mechanical complexity.
  - Wow factor <10s (25%): “Step too close” triggers immediate red + buzz + OLED
    warning.
  - Clear user value (20%): Protects art and preserves climate; instantly
    understandable.
  - Uses all components naturally (15%): Every part is essential to the protection
    story.
  ...
  Problem statement
  Museums and galleries need an unobtrusive way to keep visitors at a safe distance
  while also ensuring artwork preservation conditions. The solution must be touchless,
  reliable, and immediately readable by visitors and staff.
  ...
  | HC‑SR04 Trig | D4 | 5V + GND | 10µs trigger pulse |
  | HC‑SR04 Echo | D5 | 5V + GND | Read on Arduino only |
  | LED Green | D9 | — | 220Ω in series to GND |
  | LED Blue | D10 | — | 220Ω in series to GND |
  | LED Red | D8 | — | 220Ω in series to GND |
  ...
  void handleCommand(String line) {
    ...
    if (line.startsWith("STATE,")) {
      ...
      int b = buzz.toInt();
      if (b > 0) { buzzMs = b; buzzPending = true; }
      lastCmdMs = millis();
    }
  }
  ...
  void loop() {
    ...
    if (now - lastTelemetryMs >= 500) {
      ...
      if (ultraOk) Serial.print(lastDist, 1); else Serial.print("null");
      Serial.print(",\"temp_c\":");
      if (dhtOk) Serial.print(lastTemp, 1); else Serial.print("null");
      Serial.print(",\"hum\":");
      if (dhtOk) Serial.print(lastHum, 0); else Serial.print("null");
  ...
  def compute_state(data, now, climate_bad_since, last_state):
      ...
      if dht_ok:
          ...
          if temp is None or hum is None:
              ...
              climate_ok = (TEMP_MIN <= temp <= TEMP_MAX) and (HUM_MIN <= hum <=
  HUM_MAX)
  ...
  def main():
      ...
      while True:
          ...
          # Compute state
          state, climate_bad_since = compute_state(latest, now, climate_bad_since,
  last_state)
          if args.demo:
              state = "DEMO"
          ...
          dist_txt = f"Dist {dist:>3.0f}cm" if isinstance(dist, (int, float)) else
  "Dist --"
          th_txt = "T -- H --"
  ...
  1. Normal approach: Start far → green; step within 1m → blue; step close (<45cm) →
     red + buzz.
  2. Climate alert: Warm sensor with hand → after 15s, OLED shows “CHECK AIR,” red LED.
  3. Disconnect test: Unplug ultrasonic → “SENSORS?” with red LED; reconnect → returns
     to green within 5s.

  Demo choreography (judge‑friendly)

  - “This is GalleryGuard. It creates an invisible buffer around art and verifies
    climate.”
  ...
  “GalleryGuard is a small, silent sentinel that protects art. It watches distance and
  climate, and guides visitors instantly—no touch, no training.”

  10‑second wow line
  “Watch this: step closer and it warns you—step back and it’s calm again.”

  15‑second close + impact
  “GalleryGuard makes every exhibit safer and every visitor clearer. Simple, reliable,
  and ready for real galleries.”

  OLED microcopy (short, elegant)

  - Line1: GALLERY GUARD
  - Line2: READY / ACTIVE / STEP BACK / CHECK AIR / SENSORS?
  - Line3: Dist  42cm
  - Line4: T 22C H 48%
